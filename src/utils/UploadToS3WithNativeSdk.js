...
const UploadToS3WithNativeSdk = ({lessonId}) => {

    const [progress , setProgress] = useState(0);
    const [selectedFile, setSelectedFile] = useState(null);
    const [isUploading, setIsUploading] = useState(false);
    const {handlePut} = usePutRequest(`https://mernstacktestserver.onrender.com/lessons/${lessonId}`);
    ...
    const uploadMultipartFile =  async (file) => {
        ...
        if (!isVideo) {
            window.alert('Invalid file type. Only MP4 files are allowed.');
            return;
        }

        const target = {Bucket: S3_BUCKET, Key: `lessonVideo/${Date.now().toString()}${uuid()}-${file.name}`, Body: file}
    ...
    return <div>
        ...
        <ButtonStyled onClick={() => uploadMultipartFile(selectedFile)}> Upload Lesson</ButtonStyled>
    </div>
}
